 ...
 */
function NotesView({ group, onBack }: NotesViewProps) {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isFile && entry.name && entry.name.endsWith('.json')) {
            ...
            loadedNotes.push(JSON.parse(data) as Note);
          }
        }
  ...
  const handleNoteHover = (noteId: number | null) => {
  ...
  const handleListItemClick = (note: Note) => {
