...
interface GroupCreatorProps {
  onSave: (groupData: Omit<NoteGroup, 'id'>) => void;
  onClose: () => void;
}
 ...
 */
function GroupCreator({ onSave, onClose }: GroupCreatorProps) {
  const [name, setName] = useState<string>('');
  const [selectedColor, setSelectedColor] = useState<string>(BLUE_SHADES[0]);
  ...
  return (
    <div className="note-editor-overlay" onClick={onClose}>
      <div className="group-creator" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
