...
import type { PkmCard } from '../lib/pkmDb';
import { loadCards, syncCardLinks, getCardBacklinks, type CardBacklinkRow } from '../lib/pkmDb';
...
import { CardImageExtension } from './CardImageExtension';
import { saveImageBlob, extFromMime, cleanupOrphanImages } from '../lib/pkmImageLib';
...
export default function CardEditor({ doc, onSave, onBack, onDelete, onNavigate, hideOutline = false, transparentBg = false }: Props) {
  ...
  const [backlinks,      setBacklinks]      = useState<CardBacklinkRow[]>([]);
  const [armedDelete,    setArmedDelete]    = useState(false);
  const [allDocs,        setAllDocs]        = useState<PkmCard[]>([]);
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      syncCardLinks(doc.id, json).then(() => getCardBacklinks(doc.id).then(setBacklinks));
    },
  }, [doc.id]);
  ...
  useEffect(() => {
    ...
    setTitle(doc.title ?? '');
    setHeadings(extractHeadings(editor.getJSON()));
    getCardBacklinks(doc.id).then(setBacklinks);
  }, [doc.id]);
  ...
  useEffect(() => { loadCards().then(setAllDocs); }, [doc.id]);
  ...
  // Click on wiki-link → navigate to target card
  useEffect(() => {
  ...
  function insertWikiLink(target: PkmCard) {
          ...
          {backlinks.length > 0 && (
            <div style={{ marginTop: 60, paddingTop: 20, borderTop: '1px solid rgba(0,0,0,0.08)' }}>
              ...
              {backlinks.map(bl => (
                <button
                  key={bl.id}
                  onMouseDown={() => { if (onNavigate) onNavigate(bl.id); }}
                  style={{
