...
async function imagesDir(): Promise<string> {
  ...
  return join(docs, 'O-neash-data', 'pkm-images');
}

/** Save a Blob to the pkm-images folder. Returns the absolute path. */
...
/**
 * Delete image files in pkm-images/ that are no longer referenced
...
export async function cleanupOrphanImages(): Promise<number> {
  ...
  const notes = await db.select<{ content_json: string | null }[]>(
    `SELECT content_json FROM pkm_cards WHERE content_json IS NOT NULL AND content_json LIKE '%pkm-images%'`,
  );

  const referenced = new Set<string>();
  for (const { content_json } of notes) {
    if (!content_json) continue;
    // Match the last path segment (filename) after "pkm-images/"
    for (const m of content_json.matchAll(/pkm-images[/\\]([^"\\,\s]+)/g)) {
      referenced.add(m[1]);
    }
  }
