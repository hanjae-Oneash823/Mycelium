...
import { Home, Terminal as TerminalIcon } from "pixelarticons/react";
...
import { useXtermSession } from "../plugins/TerminalPlugin/lib/useXtermSession";
...
const TERM_ACC = '#f59e0b';

function TerminalPeekPane({ id }: { id: string }) {
  const containerRef = useRef<HTMLDivElement>(null);
  useXtermSession(id, containerRef);
  return <div ref={containerRef} style={{ width: "100%", height: "100%" }} />;
}

function TerminalPeekSection({ isOpen }: { isOpen: boolean }) {
  const setActivePlugin = usePluginStore((state) => state.setActivePlugin);
  const { terminals, peekTerminalId, setPeekTerminal } = useTerminalStore();

  const selected = terminals.find((t) => t.id === peekTerminalId) ?? terminals[0];

  useEffect(() => {
    if (isOpen && !selected && terminals.length > 0) setPeekTerminal(terminals[0].id);
  }, [isOpen, selected, terminals, setPeekTerminal]);

  const header = (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <TerminalIcon width={12} height={12} style={{ color: 'rgba(255,255,255,0.25)' }} />
      <span style={{ fontFamily: VT, fontSize: '0.72rem', letterSpacing: 3, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase' }}>
        [terminal]
      </span>
    </div>
  );
  ...
  if (terminals.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {header}
        <div style={{ fontFamily: VT, fontSize: '0.85rem', color: 'rgba(255,255,255,0.3)', letterSpacing: 0.5 }}>
          no terminals running
        </div>
        <button
          onClick={() => setActivePlugin("terminal")}
          style={{
            fontFamily: VT, fontSize: '0.85rem', letterSpacing: 1,
            background: 'none', border: `1px dashed ${TERM_ACC}44`,
            color: TERM_ACC, padding: '5px 0', cursor: 'pointer',
          }}
        >
          open Terminal app
        </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, height: "100%" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {terminals.map((t) => (
          <button
            key={t.id}
            onClick={() => setPeekTerminal(t.id)}
            style={{
              fontFamily: VT, fontSize: '0.78rem', letterSpacing: 0.5,
              background: t.id === selected?.id ? `${TERM_ACC}22` : 'none',
              border: `1px solid ${t.id === selected?.id ? TERM_ACC : 'rgba(255,255,255,0.18)'}`,
              color: t.id === selected?.id ? TERM_ACC : 'rgba(255,255,255,0.45)',
      ...
      <div style={{ flex: 1, minHeight: 0, border: '1px solid rgba(255,255,255,0.1)', background: '#080808' }}>
        {isOpen && selected && <TerminalPeekPane key={selected.id} id={selected.id} />}
      </div>
    </div>
  );
}

function AotTerminalPeekPanel() {
  const [isOpen, setIsOpen] = useState(false);
  const panelRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div
      className={`aot-term-wrapper${isOpen ? " is-open" : ""}`}
      onMouseLeave={() => setIsOpen(false)}
    >
      <div className="aot-term-panel" ref={panelRef}>
        <TerminalPeekSection isOpen={isOpen} />
      </div>
    </div>
  );
}
